...
db_api_connect <- function(d = NA,
  ...
  ## make conform to ASTR package
  # capitalize first letter for 2+ letters (ex: 'sr' -> 'Sr')
  colnames(df) <- tools::toTitleCase(colnames(df))
  # capitalize first letter for 1 letter (ex: 'u' -> 'U')
  colnames(df)[which(nchar(colnames(df))==1)] <- toupper(colnames(df)[which(nchar(colnames(df))==1)])
  # direct equivalences
  colnames(df)[which(names(df)=="Loss_on_ignition")] <- 'LOI'
  colnames(df)[which(names(df)=="Easting")] <- 'latitude'
  colnames(df)[which(names(df)=="Northing")] <- 'longitude'
  colnames(df)[which(names(df)=="Northing")] <- 'longitude'
  d[[df.label]] <- df
